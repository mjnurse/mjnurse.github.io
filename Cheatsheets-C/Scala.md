...
```scala
/* equals      */ "martin".equals("fred") // Yields: false [var/String].equals([var/String])

/* findAllIn   */ "t.* ".r.findAllIn("there's the dog")  // Yields: scala.util.matching.Regex.MatchIterator
                  "t.* ".r.findAllIn("there's the dog").foreach(println) // Yields: there's the

/* findFirstIn */ "th. ".r.findFirstIn("there's the dog") // Yields: Option[String] = Some("the ")

/* intersect   */ "Martin".intersect("tank") // Yields: "atn"

// matches        defn: [var/String].matches([regex])`
...
Function | Example | Yields | Description
---|---|---|---
equals | `"martin".equals("fred")` | `false` | `[var/String].equals([var/String])`
findAllIn | `"t.* ".r.findAllIn("there's the dog")`  | `scala.util.matching.Regex.MatchIterator`
. | `"t.* ".r.findAllIn("there's the dog").foreach(println)` | `there's the`
findFirstIn | `"th. ".r.findFirstIn("there's the dog")` | `Option[String] = Some("the ")`
intersect | `"Martin".intersect("tank")` | `"atn"`
matches | `"martin".matches("m.{4}n")` | `true`| `[var/String].matches([regex])`
split | `"a b c".split(' ')` | `Array("a", "b", "c")`
toLowerCase | `"HI".toLowerCase` |
toString | `val v = 123; v.toString` | | As toString has no parameters the () are omitted.
...
<hr>
<p class="pagedate">This page was generated by <a href=".">GitHub Pages</a>.  Page last modified: 22/01/04 16:13</p>
