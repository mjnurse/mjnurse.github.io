...
In the example below, the function `fetchData` is declared ASYNC so that it can wrap and AWAIT on a call to `axios.get`.  I use `axios.get` to fetch a web page to introduce an unpredictable delay.
