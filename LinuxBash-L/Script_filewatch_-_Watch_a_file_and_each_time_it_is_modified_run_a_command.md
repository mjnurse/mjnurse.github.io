---
title: filewatch - Watch a file and each time it is modified run a command
---

```bash
#!/bin/bash
help_text="
NAME
   filewatch - Watch a file and each time it is modified run a command.

USAGE
   filewatch <file name> <command> [<command parameters>]

DESCRIPTION
   Watch a file and each time it is modified run a command.

AUTHOR
   mjnurse.uk - 2019
"
help_line="Watch a file and each time it is modified run a command"
desc_line="Watch a file and each time it is modified run a command"

name=${0/\.\/}
try="Try '$name -h' for more information"

# Get usage from help_text.
usage=${help_text##*USAGE}
usage=${usage%%OPTIONS*}
usage=${usage%%PARAMETERS*}
usage=${usage%%DESCRIPTION*}
usage=${usage//[$'\n\r']}
usage=${usage##*   }
usage="Usage: ${usage}"

if [[ "$1" == "--help" || "$1" == "-h" || "$1" == "?" ]]; then
   echo "$help_text"
   exit
fi

if [[ "$2" == "" ]]; then
   echo "$usage"
   echo "$try"
   exit
fi

file_name=$1
shift
prev_file_date=$(date -r $file_name)
echo $prev_file_date
file_date=$prev_file_date
while [ 1 ]; do
   while [[ "$file_date" == "$prev_file_date" ]]; do
      sleep 1
      file_date=$(date -r $file_name)
   done
   prev_file_date=$file_date
   echo $prev_file_date -  Run: "$*"
   $*
done

```

<hr>
<p class="pagedate">This page was generated by <a href=".">GitHub Pages</a>.  Page last modified: 21/02/06 22:59</p>
